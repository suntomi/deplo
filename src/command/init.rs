...
impl<'a, S: shell::Shell<'a>, A: args::Args> command::Command<'a, A> for Init<'a, S> {
    ...
    fn run(&self, args: &A) -> Result<(), Box<dyn Error>> {
            ...
            match fs::remove_dir_all(self.config.infra_code_dest_path()) {
                ...
                Err(err) => { 
                    log::error!(
                        ...
                        self.config.infra_code_dest_path().to_string_lossy(), err
                    )
                },
            }
            for (k, _) in &self.config.common.release_targets {
                match fs::remove_file(self.config.endpoints_file_path(Some(k))) {
                    Ok(_) => {},
                    Err(err) => { 
                        log::error!(
                            "fail to cleanup for reinitialize: fail to remove {} with {:?}",
                            self.config.endpoints_file_path(Some(k)).to_string_lossy(), err
                        )
                    },    
                }
            }
        }
        match fs::metadata(self.config.infra_code_dest_path()) {
            Ok(_) => log::info!("infra setup scripts already copied"),
            Err(_) => {
        ...
        for (k, _) in &self.config.common.release_targets {
            match fs::metadata(self.config.endpoints_file_path(Some(k))) {
